...
GMainLoop* ClientNiceAgent::gloop = NULL;
GThread* ClientNiceAgent::gloopthread = NULL;
void ClientNiceAgent::init()
{
    ...
    gloopthread = g_thread_new("loop thread", &loop_thread, gloop);    //三个参数 分别为标识符 线程运行的函数 线程函数的参数
}
void ClientNiceAgent::release()
{
...
void* ClientNiceAgent::loop_thread(void *data)
{
...
ClientNiceAgent::ClientNiceAgent(websocket_server* pserver, connection_hdl hdl, gboolean controlling,OfferAnswerServer *offerAnswerServer, gchar* stun_addr , guint stun_port)
{
    pserver_ = pserver;
    hdl_ = hdl;
    agent = nullptr;        
   ...
   // std::string strhost = conn->get_host();
	std::string strhost ="9.134.66.120";
...
ClientNiceAgent::~ClientNiceAgent()
{
    if (agent)
    {
        if (thread_audio_)
...
int32_t ClientNiceAgent::add_stream(char* szstreamname, uint32_t componentnum)
{
...
bool ClientNiceAgent::start_gather(int32_t streamid)
{
...
void ClientNiceAgent::candidate_gathering_done(int32_t stream_id)
{
...
bool ClientNiceAgent::set_remote_sdp(const char* sdp)
{
    remote_sdp = sdp;                
    return true;
}
bool ClientNiceAgent::set_remote_candidate(const char* szcandidate)
{
...
void ClientNiceAgent::component_state_changed(int32_t streamid, uint32_t componentid, guint state)
{
...
void ClientNiceAgent::new_selected_pair_full(guint stream_id,guint component_id, NiceCandidate *lcandidate, NiceCandidate* rcandidate)
{
...
void ClientNiceAgent::nice_recv_data(int32_t streamid, uint32_t componentid, guint len, gchar *buf)
{
    ...
    {        printf("is rtp \n");
		...
		printf("PLEN:%d \n",plen);
		RtpQuePacketObj* rtpQuePacketObj=new RtpQuePacketObj();
		rtpQuePacketObj->data = buf;
		rtpQuePacketObj->length = buflen;
        this->mOfferAnswerServer->dispatchData(this->hdl_,rtpQuePacketObj);
		//g_async_queue_push(rptqueue, (gpointer)rtpQuePacketObj);
    }
    else if (is_rtcp(buf))
    {        printf("is rtcp \n");
...
bool ClientNiceAgent::pushDataToAgent(RtpQuePacketObj *rtpQuePacketObj){
	g_async_queue_push(rptqueue, (gpointer)rtpQuePacketObj);

}
bool ClientNiceAgent::send_data(int32_t streamid, uint32_t componentid, guint len, gchar *buf)
{
    int nret = nice_agent_send(this->agent, streamid, componentid, len, buf);
    return nret;
}

bool ClientNiceAgent::send_data_need_protect(int32_t streamid, uint32_t componentid, guint len, gchar *buf)
{
...
void read_send_audio(void* pdata)
{
    ClientNiceAgent* pagent = (ClientNiceAgent*)pdata;
    ...
    try
    {
		RtpQuePacketObj* rtpQuePacketObj = NULL;
        while (true)
        {        
			printf("before pop!\n");
			rtpQuePacketObj = (RtpQuePacketObj*)g_async_queue_pop(rptqueue);
			printf("after pop!\n");
			...
			memcpy(readbuf + RTP_HEADER_SIZE, rtpQuePacketObj->data + RTP_HEADER_SIZE, rtpQuePacketObj->length - RTP_HEADER_SIZE);
			pagent->send_data_need_protect(1, 1, rtpQuePacketObj->length, readbuf);
  //pagent->send_data_need_protect(1, 1, read + RTP_HEADER_SIZE, readbuf);
            seq++;
            header->seq_number = htons(seq);
            ts += 160;
...
void set_buf_rtp_video_header(uint8_t* pbuffer, uint32_t dwssrc, uint32_t dwtimestample, uint16_t dwseqnum, bool marker)
{
    ...
    rtp_hdr->seq_number = htons(dwseqnum); //���кţ�ÿ����һ��RTP����1��htons���������ֽ���ת�������ֽ���  
}
